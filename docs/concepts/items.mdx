...
PDFs and images are routed through extraction and preview pipelines so the AI can read them and the UI can render usable previews. The current stack uses Cloudflare Workflows, R2, Workers AI, Firecrawl, and conversion containers depending on the file type and configured secrets.
