...
| Workspace presence, realtime fanout, cleanup coordination | `WorkspaceRoom` Durable Object |
| Original and preview workspace file bytes | R2 `WORKSPACE_FILES` |
| AI chat threads, transcripts, and attachment bytes | PlanetScale Postgres through Drizzle and cache-disabled Hyperdrive |
| AI chat turn streaming | Plain Worker route (`/ai-chat/threads/:id`) with AI SDK `streamText` |
...
| Document conversion | Cloudflare Containers |
...
| Workspace realtime room | `src/features/workspaces/realtime/` |
