...
export function useWorkspaceMutation(workspaceId: string | null) {
  ...
  return useMutation({
    mutationFn: (event: WorkspaceEvent) => {
      ...
      const currentStateData = queryClient.getQueryData<WorkspaceStatePayload>(
      ...
      const optimisticEventsCount = events.filter(
      ...
      const currentVersion = Math.max(
        currentData?.version ?? 0,
        maxEventVersion,
        currentStateData?.activity.version ?? 0,
      );
      ...
      const adjustedBaseVersion =
        currentVersion + Math.max(0, optimisticEventsCount - 1);

      logger.debug("🚀 [MUTATION] Starting mutation:", {
        workspaceId,
        cacheVersion: currentData?.version ?? 0,
        stateVersion: currentStateData?.activity.version ?? 0,
        maxEventVersion,
        currentVersion,
        adjustedBaseVersion,
    ...
    onMutate: async (event: WorkspaceEvent) => {
      ...
      const previousState = queryClient.getQueryData<WorkspaceStatePayload>(
        workspaceStateQueryKey(workspaceId),
      );
      const statePayload =
        previousState ?? createFallbackWorkspaceStatePayload(workspaceId);

      logger.debug("⚡ [OPTIMISTIC] Applying event optimistically:", {
        eventType: event.type,
        currentVersion: previous?.version ?? statePayload.activity.version,
      });
        ...
        (old) => {
          if (!old) {
            const newState = {
              ...
              version: statePayload.activity.version,
            };
            return newState;
          }
      ...
      queryClient.setQueryData<WorkspaceStatePayload>(
        workspaceStateQueryKey(workspaceId),
        (old) => {
          return applyEventToWorkspaceStatePayload(old ?? statePayload, event);
        },
      );
    ...
    onSuccess: (data, event, context) => {
      ...
      if (event.type === "ITEM_UPDATED") {
        ...
        if (
          changesData &&
          "ocrStatus" in changesData &&
          changesData.ocrStatus
        ) {
      ...
      if (event.type === "BULK_ITEMS_PATCHED") {
        const ocrUpdates = event.payload.updates.filter((update) => {
          const dataChanges = update.changes?.data as
            | { ocrStatus?: string }
            | undefined;
        ...
        if (ocrUpdates.length > 0) {
          logger.debug("[OCR/UPDATE] BULK_ITEMS_PATCHED persisted", {
            ...
            statuses: ocrUpdates.map(
              (update) =>
                (update.changes?.data as { ocrStatus?: string } | undefined)
                  ?.ocrStatus,
            ),
            version: data.version,
          });
      ...
      if (data.conflict) {
        ...
        if (currentRetries < MAX_RETRY_ATTEMPTS) {
          logger.warn(
            `⚠️ [CONFLICT] Version conflict detected, auto-retrying (attempt ${currentRetries + 1}/${MAX_RETRY_ATTEMPTS})...`,
          );
          ...
          queryClient.setQueryData<EventResponse>(
            ...
            (old) => {
              ...
              const filteredEvents = old.events.filter(
                (e) => e.id !== event.id,
              );
          ...
          queryClient
            ...
            .then(() => {
              ...
              logger.debug(
                "🔄 [RETRY] Retrying event after refetch:",
                event.type,
              );
              ...
              queryClient.setQueryData<WorkspaceStatePayload>(
                workspaceStateQueryKey(workspaceId),
                (old) =>
                  old ? applyEventToWorkspaceStatePayload(old, event) : old,
              );
              ...
              appendWorkspaceEvent({
                ...
                .then((retryResult) => {
                  if (retryResult.conflict) {
                      ...
                      logger.error(
                        "❌ [RETRY] Retry failed with conflict, giving up",
                      );
          ...
          logger.error(
            `❌ [CONFLICT] Max retries (${MAX_RETRY_ATTEMPTS}) exceeded for event ${event.id}`,
          );
        ...
        queryClient.setQueryData<WorkspaceStatePayload>(
          ...
          (old) =>
            old
              ? confirmWorkspaceStatePayloadVersion(old, data.version, event)
              : old,
        );
