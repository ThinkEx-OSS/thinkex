...
import { type Block } from "./blocknote-shared";
...
const LazyMathField = memo(
  function LazyMathField({
    latex,
    isInline = false,
  }: {
    latex: string;
...
export const BlockNotePreview = memo(
  function BlockNotePreview({
    blocks,
    className,
    isScrollLocked = true,
    scrollParent,
  }: BlockNotePreviewProps) {
...
export const PreviewBlock = memo(
    ...
    switch (block.type) {
      ...
      case "numberedListItem": {
        ...
        return (
          <div className="mb-1">
            <div className="flex gap-2 items-center">
              <span className="select-none text-muted-foreground font-mono text-xs">
                {listNumber}.
              </span>
              <div className="flex-1 min-w-0 break-words">
                <p>
        ...
        return (
          <div className="mb-1">
            <div className="flex gap-2 items-start">
              ...
              <div
                className={`flex-1 min-w-0 break-words ${block.props.checked ? "line-through text-muted-foreground" : ""}`}
              >
                <p>
                  <InlineContent content={content} />
                </p>
  ...
  (prevProps, nextProps) => {
    ...
    if (
      ...
      return false;
    if (
      JSON.stringify(prevProps.block.props) !==
      JSON.stringify(nextProps.block.props)
    )
      return false;
    if (
      JSON.stringify(prevProps.block.children) !==
      JSON.stringify(nextProps.block.children)
    )
      return false;
    ...
    return (
      <>
        {content.map((item, index) => {
          ...
          if (item.type === "text") {
            return (
              <StyledText key={index} text={item.text} styles={item.styles} />
            );
          }
    ...
    for (let i = 0; i < prevProps.content.length; i++) {
      ...
      if (prev.type === "link") {
        ...
        for (let j = 0; j < prev.content.length; j++) {
          ...
          if (JSON.stringify(prevSub.styles) !== JSON.stringify(nextSub.styles))
            return false;
        }
      }
    }
...
const StyledText = memo(
  function StyledText({ text, styles }: { text: string; styles: any }) {
    const element = useMemo(() => {
      ...
      if (styles?.code) {
        el = (
          <code className="bg-foreground/10 px-1 py-0.5 rounded font-mono text-foreground dark:bg-white/10 dark:text-white">
            {el}
          </code>
        );
      }
      ...
      if (styles?.backgroundColor) {
        el = (
          <span style={{ backgroundColor: styles.backgroundColor }}>{el}</span>
        );
      }
