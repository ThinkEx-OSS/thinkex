...
export interface AssistantThreadSelectionProps {
  ...
  /** Scroll/root container for selection bounds (e.g. `.chat-v2-thread-viewport`) */
 ...
 * - Assistant body only (`.chat-v2-assistant-message-content`).
...
export function AssistantThreadSelection({
  ...
  const commitSelection = useCallback(() => {
    requestAnimationFrame(() => {
      ...
      if (
        !startEl?.closest(".chat-v2-assistant-message-content") ||
        !endEl?.closest(".chat-v2-assistant-message-content")
      ) {
        clear();
        return;
      }
