...
import {
  ...
  ImageRoot,
  ImageZoom,
} from "@/components/chat-v2/parts/image";
...
export const ExecuteCodeToolUI: React.FC<ToolUIProps<{ code: string }, CodeExecuteResult>> = ({ state, input, output }) => {
  ...
  return (
    <ToolUIErrorBoundary componentName="ExecuteCode">
      {state === "input-streaming" || state === "input-available" ? (
        ...
        <ExecuteCodeResult args={args} result={output} />
      )}
    </ToolUIErrorBoundary>
  );
};
