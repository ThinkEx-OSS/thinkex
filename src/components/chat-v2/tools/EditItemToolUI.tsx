...
const EditItemReceipt = ({ result, state }: EditItemReceiptProps) => {
    ...
    <div className="my-1 flex flex-col gap-2">
      ...
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div
            className={cn(
              state === "output-available" ? "text-blue-400" : "text-red-400",
            )}
          >
            {state === "output-available" ? (
...
export const EditItemToolUI: React.FC<ToolUIProps<EditItemArgs, WorkspaceResult>> = ({ input, output, state, errorText }) => {
  const args = input as EditItemArgs;
  ...
  const statusErrorMessage =
    errorText ?? "An error occurred while editing";
