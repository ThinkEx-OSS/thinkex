...
export const URLContextToolUI: React.FC<ToolUIProps<{
  urls?: string[];
  jsonInput?: string;
}, ProcessUrlsResult>> = ({ input, state, output }) => {
  const isRunning = state === "input-streaming" || state === "input-available";
  const isComplete = state === "output-available";

  const parsedResult = output != null ? parseURLContextResult(output) : null;
  type Meta = {
  ...
  const normalizedArgs = normalizeProcessUrlsArgs(input);
