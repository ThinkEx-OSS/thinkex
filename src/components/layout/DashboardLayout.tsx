...
import { AiComingSoonPanel } from "@/components/layout/AiComingSoonPanel";
      ...
      {effectiveChatMaximized ? (
        <div className="relative flex-1 h-full z-10">
          <div className="flex-1 h-full">
            <AiComingSoonPanel
              onCollapse={() => {
                if (effectiveChatMaximized) setIsChatMaximized?.(false);
                setIsChatExpanded?.(false);
              }}
              isMaximized={effectiveChatMaximized}
              onToggleMaximize={() =>
                setIsChatMaximized?.(!effectiveChatMaximized)
              }
            />
          </div>
        </div>
