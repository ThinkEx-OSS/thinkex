...
import type { PromptBuilderAction } from "@/components/chat/PromptBuilderDialog";

/**
 * Quick actions shown in a hover-activated pill row above the composer when
 * the thread is non-empty and the input is blank. Each entry either opens
 * `PromptBuilderDialog` with a preset `PromptBuilderAction`, or nests a small
 * dropdown of sub-actions (e.g. "Learn" → Flashcards / Quiz).
 */
