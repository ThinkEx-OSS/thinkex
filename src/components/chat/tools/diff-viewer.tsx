...
/**
 * Minimal prop shape for the `SyntaxHighlighter` slot — the diff viewer
 * only receives the highlighter as a component, it never calls into it.
 */
interface SyntaxHighlighterProps {
  language: string;
  code: string;
  components?: { pre?: React.ComponentType<unknown> };
}
            ...
            {renderMarkdown && viewMode === "unified" ? (
              groupLinesByType(file.lines).map((chunk, chunkIndex) => {
                ...
                return (
                  <div
                    key={chunkIndex}
                    data-slot="diff-viewer-chunk"
                    data-type={chunk.type}
                    className={cn(
                      "px-4 py-2 border-l-4",
                      chunk.type === "add" &&
                        "bg-[var(--diff-add-bg,_rgba(46,160,67,0.15))] border-l-[var(--diff-add-text,_#1a7f37)] dark:border-l-[var(--diff-add-text-dark,_#3fb950)]",
                      chunk.type === "del" &&
                        "bg-[var(--diff-del-bg,_rgba(248,81,73,0.15))] border-l-[var(--diff-del-text,_#cf222e)] dark:border-l-[var(--diff-del-text-dark,_#f85149)]",
                      chunk.type === "normal" && "border-l-transparent"
                    )}
                  >
                    <div
                      className={cn(
                        ...
                        chunk.type === "add" &&
                          "text-[var(--diff-add-text,_#1a7f37)] dark:text-[var(--diff-add-text-dark,_#3fb950)]",
                        chunk.type === "del" &&
                          "text-[var(--diff-del-text,_#cf222e)] dark:text-[var(--diff-del-text-dark,_#f85149)]",
                        chunk.type === "normal" && "text-muted-foreground"
                      )}
                    >
                      {label}
                    </div>
                    <StreamdownMarkdown className="[&_.streamdown-content]:text-inherit [&_.streamdown-content]:leading-snug">
                      {chunk.content}
                    </StreamdownMarkdown>
                  </div>
                );
