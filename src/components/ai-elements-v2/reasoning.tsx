...
function useMirroredState<T>({
  ...
  const [value, setValue] = useState<T>(prop ?? defaultProp);
  ...
  const setMirroredValue = useCallback(
    (nextValue: T) => {
      if (!isControlled) {
        setValue(nextValue);
      }
  ...
  return [isControlled ? (prop as T) : value, setMirroredValue] as const;
}
...
export const Reasoning = memo(
  ({
    ...
    const [isOpen, setIsOpen] = useMirroredState<boolean>({
      prop: open,
      defaultProp: resolvedDefaultOpen,
      onChange: onOpenChange,
    });
    const [duration, setDuration] = useMirroredState<number | undefined>({
      prop: durationProp,
