...
export class PostgresWorkspaceFiles {
	constructor(
	...
	async createFileFromUpload(
		...
		const [source, preview] = await Promise.all([
			...
			this.bucket.head(input.preview.objectKey),
		]);
	...
	async readFileExtraction(
		...
		return await withWorkspaceDatabase(async (db) => {
			...
			const [projection] = await db
				...
				.where(
					and(
						eq(workspaceItemExtractions.workspaceId, this.workspaceId),
	...
	async publishPages(input: {
		...
		const publication = await withWorkspaceTransaction(async (transaction) => {
			...
			await transaction
				.delete(workspaceItemPages)
				.where(eq(workspaceItemPages.itemId, input.itemId));
	...
	async readPages(input: { itemId: string; pageNumbers: number[] }) {
