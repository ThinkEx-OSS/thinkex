...
export const workspaceToolDefinitions = [
	...
	defineWorkspaceTool({
		...
		access: "read",
		description:
			"Read ThinkEx documents, recordings, flashcard sets, quizzes, and extracted files. mode start returns the first chunk and its location. Continue with mode entries for document blocks, five-minute recording transcript sections, cards, or questions; use mode pages for file pages. Each result says which numbered units it returned and the total. Each result also carries the item's durable ref, while blocks, cards, and questions carry revisioned unit refs to copy exactly into edits or citations. Use mode ref to read one addressed unit in full. Flashcard and quiz reads include the current user's study progress.",
