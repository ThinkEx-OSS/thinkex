...
import {
  getWorkspaceEventVersion,
  loadWorkspaceItemsState,
...
async function appendWorkspaceEventOrThrow(
  workspaceId: string,
  event: WorkspaceEvent,
  conflictMessage: string,
): Promise<{ version: number; conflict: boolean }> {
  const result = await appendWorkspaceEventAtCurrentVersionWithProjection({
    workspaceId,
    event,
  });
    ...
    async () => {
      try {
        ...
        if (action === "create") {
          ...
          const currentState = await loadWorkspaceItemsState(params.workspaceId);
          if (
            hasDuplicateName(
              currentState.items,
              item.name,
          ...
          // Get initial version
          baseVersion = await getWorkspaceEventVersion(params.workspaceId);
          appendResult = { version: baseVersion, conflict: false };

          while (retryCount <= maxRetries) {
            appendResult = await appendWorkspaceEventWithProjection({
        ...
        if (action === "bulkCreate") {
          ...
          const appendResult = await appendWorkspaceEventOrThrow(
            params.workspaceId,
            event,
            "Workspace was modified by another user, please try again",
          );
        ...
        if (action === "updateQuiz") {
          ...
          logger.debug("📝 [UPDATE-QUIZ-DB] Created event:", {
            ...
            questionsInPayload: (event.payload.changes?.data as any)?.questions
              ?.length,
          });
        ...
        if (action === "edit") {
          ...
          const cards: FlashcardItem[] = data.cards ?? [];
            ...
            if (rename) {
              if (
                ...
                return {
                  success: false,
                  message: `A flashcard deck named "${rename}" already exists in this folder`,
                };
              }
              changes.name = rename;
            }
          ...
          if (existingItem.type === "quiz") {
            ...
            if (rename) {
              if (
                ...
                return {
                  success: false,
                  message: `A quiz named "${rename}" already exists in this folder`,
                };
              }
              changes.name = rename;
            }
          ...
          if (existingItem.type === "document") {
            ...
            if (rename) {
              if (
                ...
                return {
                  success: false,
                  message: `A document named "${rename}" already exists in this folder`,
                };
              }
              changes.name = rename;
            }
          ...
          if (existingItem.type === "pdf") {
            ...
            if (
              ...
              return {
                success: false,
                message: `A PDF named "${rename}" already exists in this folder`,
              };
            }
