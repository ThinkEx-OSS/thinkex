...
export function createDocumentTool(ctx: WorkspaceToolContext) {
  return withSanitizedModelOutput(
    tool({
      description:
        "Create a document card. Documents use a rich TipTap editor with full markdown support (headings, lists, tables, math, etc.). Use this for longer-form structured content.",
      inputSchema: zodSchema(
        z.object({
          ...
          content: z
            .string()
            .describe(
              "The markdown body content. CRITICAL: DO NOT repeat the title in content — the title is displayed separately. Start with subheadings or body text only.",
            ),
          sources: z
            .array(sourceSchema)
            .optional()
            .describe("Optional sources from web search or deep research"),
          folderName: z
            .string()
            .optional()
            .describe(
      ...
      execute: async ({ title, content, sources, folderName }) => {
        ...
        logger.debug(
          "🎯 [ORCHESTRATOR] Delegating to Workspace Worker (create document):",
          {
            title,
        ...
        if (folderName !== undefined) {
          try {
            ...
            targetFolderId = resolveFolderByName(
              state,
              folderName,
              ctx.activeFolderId,
            );
...
export function createDeleteItemTool(ctx: WorkspaceToolContext) {
  return withSanitizedModelOutput(
    tool({
      description:
        "Delete one or more workspace items by name. This permanently removes them from the workspace.",
      inputSchema: zodSchema(
        z.object({
          itemNames: z
            .array(z.string())
          ...
          itemName: z
            ...
            .optional()
            .describe(
              "Deprecated: use itemNames instead. Single item name for backward compatibility.",
            ),
        }),
      ),
      strict: true,
      execute: async (rawInput: {
        ...
        const itemNames = rawInput.itemNames?.length
          ? rawInput.itemNames
          : rawInput.itemName
            ? [rawInput.itemName]
            : [];
        if (itemNames.length === 0) {
          return {
            success: false,
            message:
              "At least one item name is required. Provide itemNames array or itemName string.",
          };
        }
        ...
        try {
          ...
          for (const itemName of itemNames) {
            const matchedItem = resolveItem(remainingState, itemName);
            if (!matchedItem) {
            ...
            try {
              ...
              remainingState = remainingState.filter(
