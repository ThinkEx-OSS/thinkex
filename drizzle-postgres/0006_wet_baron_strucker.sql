...
CREATE TABLE "ai_chat_messages" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "ai_chat_messages_thread_id_id_pk" PRIMARY KEY("thread_id","id"),
	CONSTRAINT "ai_chat_messages_role_check" CHECK ("ai_chat_messages"."role" in ('user', 'assistant', 'system')),
	CONSTRAINT "ai_chat_messages_status_check" CHECK ("ai_chat_messages"."status" in ('complete', 'interrupted', 'error'))
);
--> statement-breakpoint
...
CREATE INDEX "ai_chat_attachments_thread_id_idx" ON "ai_chat_attachments" USING btree ("thread_id");--> statement-breakpoint
